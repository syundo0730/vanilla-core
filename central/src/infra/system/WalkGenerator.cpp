...
void WalkGenerator::run() {
	const int step = 10;
	LinearInvertedPendulum lip(step);
	lip.setWalkParam(1, Eigen::Vector2d(0, 0.2));
	lip.setWalkParam(2, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(3, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(4, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(5, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(6, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(7, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(8, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(9, Eigen::Vector2d(0.2, 0.2));
	lip.setWalkParam(10, Eigen::Vector2d(0, 0.2));
	const double Tc = 0.8, z_c = 0.8;
	lip.setInitialParam(Tc, z_c, Eigen::Vector2d(0, 0));
	lip.execOptimization(10, 1);
	...
	for (int n = 0; n <= step; ++n) {
		const Eigen::Vector2d leg_pos = lip.getLegPosition(n);
		for (double t =0; t < Tc; t += dt) {
			// current
			auto cog = lip.calcCogTrajectory(t, n);
			...
			// dist
			auto cog_d = lip.calcCogTrajectory(t + time_shift, n);
