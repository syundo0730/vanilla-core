...
/** Default constructor.
 */
I2Cdev::I2Cdev(mraa::I2c &_i2c) : i2c(_i2c) {
}
...
int8_t I2Cdev::readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *data, uint16_t timeout)
{
	...
	uint8_t* readData = new uint8_t[length];
	int len = i2c.readBytesReg(regAddr, readData, length);
...
bool I2Cdev::writeBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, const uint8_t *data)
{
	...
	i2c.address(devAddr);
	mraa::Result res = i2c.write(writeData, length+1);
	delete[] writeData;
